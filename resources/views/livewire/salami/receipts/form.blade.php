<section class="space-y-6">
    ...
    <form wire:submit="saveDraft" class="space-y-6">
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between"><div><h2 class="font-bold text-slate-950">بنود الاستلام</h2><p class="mt-1 text-sm text-slate-600">لا يمكن اختيار الصنف أكثر من مرة في الإيصال نفسه.</p></div><button type="button" wire:click="addItem" class="rounded-lg border border-amber-300 bg-amber-50 px-4 py-2 text-sm font-bold text-amber-800 hover:bg-amber-100">إضافة صنف</button></div>
                ...
                <article wire:key="receipt-item-{{ $index }}" class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="space-y-5 p-5">
                        <div class="grid gap-4 sm:grid-cols-2">
                            ...
                            <div class="rounded-xl bg-slate-50 p-3 text-sm text-slate-600"><p><span class="font-bold text-slate-700">الوحدة:</span> {{ $selectedProduct?->unit ?? '—' }}</p><p class="mt-1"><span class="font-bold text-slate-700">الرصيد الحالي:</span> {{ $selectedProduct?->current_quantity ?? '—' }}</p></div>
                        ...
                        <div class="grid gap-3 rounded-xl border border-amber-100 bg-amber-50 p-4 sm:grid-cols-2 lg:grid-cols-5">
                            ...
                            <div><p class="text-xs font-bold text-amber-800">المضاف للمخزون</p><p class="mt-1 text-lg font-bold text-amber-950">{{ $preview['accepted_quantity'] }}</p></div>
                            ...
                            <div><p class="text-xs font-bold text-amber-800">الرصيد المتوقع</p><p class="mt-1 text-lg font-bold text-amber-950">{{ $selectedProduct && $preview['valid'] ? \App\Support\Quantity::from($selectedProduct->current_quantity)->plus(\App\Support\Quantity::from($preview['accepted_quantity']))->toString() : '—' }}</p></div>
                        </div>
        ...
        <div class="sticky bottom-3 flex flex-wrap gap-3 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-lg backdrop-blur">
            ...
            <button wire:click="confirm" wire:loading.attr="disabled" type="button" class="rounded-lg bg-amber-500 px-5 py-2.5 text-sm font-bold text-amber-950 hover:bg-amber-400">اعتماد وإضافة للمخزون</button>
