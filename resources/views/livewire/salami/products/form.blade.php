<section class="mx-auto max-w-3xl space-y-6">
    ...
    <form wire:submit="save" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <label class="block text-sm font-bold text-slate-700">الوحدة<input wire:model="unit" type="text" placeholder="كرتونة" class="mt-2 w-full rounded-lg border-slate-300 focus:border-amber-500 focus:ring-amber-500">@error('unit')<span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>@enderror</label>
