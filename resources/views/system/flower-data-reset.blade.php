@extends('layouts.app', ['title' => 'إزالة بيانات الورد'])
    ...
    <section class="mx-auto max-w-2xl py-4 sm:py-10">
        <div class="overflow-hidden rounded-2xl border border-rose-200 bg-white shadow-sm">
            <div class="border-b border-rose-200 bg-rose-50 px-5 py-5 sm:px-7">
                ...
                <h1 class="mt-1 text-2xl font-bold text-slate-950">إزالة بيانات الورد فقط</h1>
            ...
            <div class="space-y-5 px-5 py-6 sm:px-7">
                <div class="rounded-xl border border-rose-100 bg-rose-50 p-4 text-sm leading-7 text-rose-950">
                    <p class="font-bold">سيتم حذف بيانات الورد فقط:</p>
                    <p class="mt-1">أنواع الورد، الاستلامات، الفواتير، الدفعات، الخروجات، التالف، التسويات وحركات المخزون.</p>
                    <p class="mt-2 font-bold">لن يتم حذف الموردين أو حسابات المستخدمين أو أي بيانات خاصة بالسلامي.</p>
                </div>

                <form method="POST" action="{{ route('system.flower-data-reset.destroy') }}" class="space-y-4">
                    @csrf
                    @method('DELETE')

                    <label class="block text-sm font-bold text-slate-700">
                        للتأكيد اكتب: <span dir="ltr" class="font-mono">مسح بيانات الورد</span>
                    ...
                    <div class="flex flex-wrap gap-3 border-t border-slate-100 pt-5">
                        <button type="submit" class="rounded-lg bg-rose-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-rose-700">إزالة بيانات الورد فقط</button>
