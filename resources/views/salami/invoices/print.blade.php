...
<html lang="ar" dir="rtl">
    <head>
        ...
        <style>
            * { box-sizing: border-box; }
            body { margin: 0; background: #f8fafc; color: #0f172a; direction: rtl; font-family: 'IBM Plex Sans Arabic', Tahoma, Arial, sans-serif; }
    ...
    <body>
        <main class="page">
            ...
            <section class="meta"><div><span class="label">المحل / العميل</span><span class="value">{{ $invoice->customer->name }}</span></div><div><span class="label">المندوب</span><span class="value">{{ $invoice->deliveryAgent?->name ?? '—' }}</span></div><div><span class="label">حالة الفاتورة</span><span class="value">{{ $invoice->status->value==='confirmed'?'معتمدة':($invoice->status->value==='cancelled'?'ملغاة':'مسودة') }}</span></div><div><span class="label">نوع الدفع</span><span class="value">{{ $invoice->payment_type->value==='cash'?'نقدي':($invoice->payment_type->value==='credit'?'آجل':'جزئي') }}</span></div><div><span class="label">حالة السداد</span><span class="value">{{ $invoice->payment_status->value==='paid'?'مسددة':($invoice->payment_status->value==='partial'?'جزئية':'غير مسددة') }}</span></div></section>
            <table><thead><tr><th>#</th><th>الصنف</th><th>وحدة العملية</th><th>الكمية</th><th>خصم المخزون</th><th>سعر الوحدة</th><th>الإجمالي</th></tr></thead><tbody>@foreach($invoice->items as $item)<tr><td>{{ $loop->iteration }}</td><td>{{ $item->product_name }}</td><td>{{ $item->unit }}@if($item->unit === 'صندوق') ({{ $item->conversion_factor }} قطعة/صندوق)@endif</td><td>{{ $item->quantity }}</td><td>{{ $item->stock_quantity }} {{ $item->product?->unit ?? 'قطعة' }}</td><td>{{ $item->unit_price }}</td><td>{{ $item->line_total }}</td></tr>@endforeach</tbody></table>
