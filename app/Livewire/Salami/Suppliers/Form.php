...
class Form extends Component
{
    ...
    public string $moduleScope = 'salami';
    ...
    public function mount(?Supplier $supplier = null): void
    {
        ...
        if ($supplier instanceof Supplier && ! in_array($supplier->module_scope, [SupplierScope::Salami, SupplierScope::Both], true)) {
            abort(404);
        }
    ...
    public function save(): mixed
    {
        ...
        if ($this->supplier instanceof Supplier) {
            ...
            session()->flash('status', 'تم تحديث بيانات المورد.');
        } else {
            $this->supplier = Supplier::query()->create([...$attributes, 'created_by' => auth()->id()]);
            session()->flash('status', 'تم إنشاء المورد.');
        }

        return $this->redirectRoute('salami.suppliers.index', navigate: true);
    }
    ...
    protected function rules(): array
    {
        return [
            ...
            'moduleScope' => ['required', Rule::in([SupplierScope::Salami->value, SupplierScope::Both->value])],
    ...
    {
        return view('livewire.salami.suppliers.form');
    }
}
